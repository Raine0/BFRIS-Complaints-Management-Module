...
class PDF extends FPDF
{
    ...
    function residentCount()
    {
        require("../../../../db_conn.php");
        $where = $_GET['msg'];
        $populationSql = "SELECT * FROM resident $where";
...
$width_cell = array(20, 25, 35, 20, 10, 50, 30);
if ($residentCount > 0) {
    $residents = $residentStatement->fetch(PDO::FETCH_ASSOC);

    foreach ($residents as $resident) {
        $pdf->SetFont('Montserrat-Regular', '', 9);

        $pdf->Cell($width_cell[0], 6, $resident['resident_id'], 1, 0, 'C');
        $pdf->Cell($width_cell[1], 6, $resident['last_name'], 1, 0, 'L');
        $pdf->Cell($width_cell[2], 6, $resident['first_name'], 1, 0, 'L');
        $pdf->Cell($width_cell[3], 6, $resident['mid_name'], 1, 0, 'L');
        $pdf->Cell($width_cell[4], 6, $resident['suffix'], 1, 0, 'L');
        $pdf->Cell($width_cell[5], 6, $resident['purok'] . ", " . $resident['street'] . ", " . $resident['lot_number'], 1, 0, 'L');
        $pdf->Cell($width_cell[6], 6, $resident['phone_number'], 1, 0, 'C');
        $pdf->Ln();
...
date_default_timezone_set("Asia/Manila");
$filename = 'Residents-' . date("m-d-Y h:i:s a")   . '.pdf';
