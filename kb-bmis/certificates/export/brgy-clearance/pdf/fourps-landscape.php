...
class PDF extends FPDF
{
    ...
    function Header()
    {
        ...
        if ($this->PageNo() === 1) {
            ...
            // Font
            $this->SetFont('Montserrat-Bold', '', 30);
            ...
            // Title
            $this->Cell(140, 30, 'BARANGAY CLEARANCE - 4PS', 0, 0, 'C');
    ...
    {
        require("./../../../../../db_conn.php");
        $fourpsQuery = "SELECT * FROM resident, barangay_clearance WHERE resident.resident_id = barangay_clearance.resident_id AND resident.fourps_status = :fourps_status";
    ...
    function TableHeader()
    {
        ...
        $width_cell = array(10, 55, 45, 30, 30, 20, 55, 30);
        $this->SetFont('Montserrat-Bold', '', 10);
...
$clearanceQuery2 = "SELECT * FROM brgy_clearance_view WHERE fourps_status = :fourps_status";

$clearanceStatement2 = $pdo->prepare($clearanceQuery2);
$clearanceStatement2->bindValue(':fourps_status', '4Ps');
$clearanceStatement2->execute();
$clearancCount2 = $clearanceStatement2->rowCount();

$width_cell = array(10, 55, 45, 30, 30, 20, 55, 30);
if ($clearancCount2 > 0) {

    while ($clearance = $clearanceStatement2->fetch(PDO::FETCH_ASSOC)) {
...
$filename = 'Fatima-Brgy-Clearance-4Ps (' . date("m-d-Y") . ').pdf';
